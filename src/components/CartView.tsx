...
const CartView: React.FC<CartViewProps> = ({ setView }) => {
  ...
  // ✅ Safer product styling handler
  const getProductStyling = (productName?: string) => {
    if (!productName || typeof productName !== "string") {
      return { color: "bg-gray-400", text: "Item" };
    }
    ...
    switch (safeName) {
      ...
      default:
        ...
        return { color: "bg-gray-500", text: words[0] || "Item" };
    }
  };
              ...
              return (
                <div key={item.id} className="flex items-center justify-between py-6">
                  <div className="flex items-center gap-4 flex-grow">
                    <div
                      className={`w-16 h-16 rounded-lg ${color} flex items-center justify-center flex-shrink-0`}
                    >
