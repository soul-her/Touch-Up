      ...
      <div className="bg-white p-8 rounded-xl shadow-xl mb-8">
        <div className="flex flex-col items-center md:flex-row md:items-start md:gap-8">
            ...
            <div className="text-center md:text-left mt-4 md:mt-0">
                <h1 className="text-3xl font-bold text-gray-800">{currentUser.displayName || 'User'}</h1>
                    ...
                    <div className="flex justify-between items-start mb-3">
                        <div>
                           ...
                           <p className="text-sm text-gray-500">
                               Placed on {order.createdAt ? new Date(order.createdAt.seconds * 1000).toLocaleDateString() : 'N/A'}
                           </p>
                        </div>
                        <span className={`text-sm font-bold px-3 py-1 rounded-full ${getStatusColor(order.status)}`}>
                            {order.status}
                        </span>
                    </div>
                    {order.driverName && <p className="text-sm text-gray-600 mb-2">Driver: <span className="font-medium">{order.driverName}</span></p>}
